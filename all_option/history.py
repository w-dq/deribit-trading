...
def run_analysis(name_list):
    res = []
    for ins_name in tqdm(name_list):
        params = {
            "count": 10000,
            "include_old": True,
...
if __name__ == "__main__":
    ...
    tag = True
    if args.instruments:
        response = requests.get("https://history.deribit.com/api/v2/public/get_instruments?currency=BTC&include_old=true")
        if response.status_code == 200:
    ...
    #     df.to_csv("expired_btc_time_summary.csv")
